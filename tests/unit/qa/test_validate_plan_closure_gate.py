...
def test_plan_closure_gate_strict_mode_does_not_accept_open_items():
    ...
    assert result["summary"]["open"] == 6
    assert all(item["status"] == "open" for item in result["gates"])
    assert all(item["strict_errors"] for item in result["gates"])
...
def test_plan_closure_gate_cli_requires_allow_open_for_zero_exit():
    ...
    assert strict_payload["summary"]["open"] == 6

    assert allowed.returncode == 0
    allowed_payload = json.loads(allowed.stdout)
    assert allowed_payload["acceptable"] is True
    assert allowed_payload["summary"]["open"] == 6
