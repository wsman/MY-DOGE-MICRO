...
class FakeLifecycleService(IRunLifecycleService):
    ...
    async def create_run(self, scope, request):
        self.calls.append(("create_run", scope, request))
    ...
    async def run_to_pause_or_completion(self, scope, run_id):
        self.calls.append(("run_to_pause_or_completion", scope, run_id))
        run = self._runs.get(run_id)
        if run:
            run.status = RunStatus.COMPLETED
        return run

    async def queue_run(self, scope, run_id, reason="queued"):
        self.calls.append(("queue_run", scope, run_id, reason))
        run = self._runs.get(run_id)
        if run:
    ...
    async def finalize_cancelled(self, scope, run_id):
        self.calls.append(("finalize_cancelled", scope, run_id))
        run = self._runs.get(run_id)
        if run:
    ...
    async def record_failure(self, scope, run_id, message):
        self.calls.append(("record_failure", scope, run_id, message))
        run = self._runs.get(run_id)
        if run:
...
class FakeStepper(IRunStepper):
    ...
    async def step(self, scope, run_id):
        self.calls.append(("step", scope, run_id))
...
class FakeApprovalCoordinator(IApprovalCoordinator):
    ...
    async def resolve(self, scope, run_id, approval_id, approved):
        self.calls.append(("resolve", scope, run_id, approval_id, approved))
        return AgentRun.create(workflow="test", question="Q")
...
async def test_kernel_create_run_delegates_to_lifecycle_service(kernel):
    ...
    assert kernel._lifecycle.calls[0][1] == TenantScope.local()
...
async def test_kernel_run_to_pause_or_completion_delegates_to_lifecycle_service(kernel_with_run):
    ...
    assert kernel._lifecycle.calls[0][1] == TenantScope.local()
    assert kernel._lifecycle.calls[0][2] == run.run_id
...
async def test_kernel_step_delegates_to_stepper(kernel):
    ...
    assert kernel._stepper.calls[0][1] == TenantScope.local()
    assert kernel._stepper.calls[0][2] == "run-1"
