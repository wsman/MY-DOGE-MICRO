...
def test_slot_signing_repository_revoke_is_idempotent(tmp_path) -> None:
    ...
    assert second.successor_key_id == "ops-key-v2"

    with sqlite3.connect(db) as conn:
        rows = conn.execute(
            "SELECT key_id, reason, actor_hash, successor_key_id FROM slot_signer_revocations"
        ).fetchall()
    assert rows == [("ops-key", "second", "actor-b", "ops-key-v2")]
