...
> **Linked Documents**: `design/cdd/module-index.md`, `docs/ux/interaction-pattern-library.md`
...
> audit, and whenever a new game system is added to `module-index.md`.
>
> **Product adaptation**: For APIs, CLIs, web apps, desktop/mobile products,
> libraries, or data tools, keep the same project-wide commitment structure.
> Replace player-facing gameplay examples with user workflow, API/CLI, docs,
> data-entry, and operational feedback examples. WCAG remains relevant for web
> and docs; terminal output, machine-readable errors, and migration safety become
> first-class accessibility surfaces for developer and operator products.
...
| Product Surface | Basic | Standard | Comprehensive | Exemplary |
|----------------|-------|----------|---------------|-----------|
| Web app | WCAG AA contrast for primary flows, visible focus states, no color-only status | Full keyboard navigation, form errors tied to fields, reduced motion support | Screen reader QA for core workflows, high contrast mode, accessible charts/tables | External accessibility audit and assistive-tech user testing |
| CLI | Clear stdout/stderr split, nonzero exit codes, no color-only failures | `--no-color`, `--json`, progress messages that work in CI logs | Structured remediation output, accessible terminal width behavior, predictable prompts | Tested with screen readers and common terminal/shell combinations |
| API / SDK | Machine-readable errors with stable codes and docs links | Idempotency/retry guidance, pagination and rate-limit errors documented | SDK examples include accessible error handling and telemetry guidance | Integrator usability testing across novice and expert users |
| Data / migration workflow | Dry-run mode and readable row-level errors | Reversible migration plan, rejected-row export, resumable jobs | Operator dashboard or logs expose state without relying on color | External operational readiness review with accessibility scenarios |
| Documentation | Headings, alt text, readable code blocks, link text with context | Task-oriented docs with copyable commands and troubleshooting paths | Searchable reference, examples for failure modes, plain-language concepts | Accessibility-reviewed docs with user testing for onboarding tasks |
...
**Product features explicitly in scope (if product project)**:
- [e.g., "Keyboard-only completion of signup, billing, and workspace invite flows"]
- [e.g., "`--json` output for all CLI commands used in CI"]
- [e.g., "Stable API error codes with remediation docs links"]
- [e.g., "Migration dry-run and rejected-row export for data imports"]

**Product features explicitly out of scope (if product project)**:
- [e.g., "Voice control for admin dashboard — not required for MVP, tracked as v2 accessibility enhancement"]
- [e.g., "Full SDK sample coverage in every language — MVP covers primary SDK only"]
...
### Product Color / Status Audit

| Location | Status Signal | What It Communicates | Non-Color Backup | Status |
|----------|---------------|---------------------|------------------|--------|
| [Web dashboard job row] | [Green = completed, red = failed] | [Job outcome] | [Status text, icon shape, and screen-reader label] | [Not Started] |
| [CLI progress output] | [Red terminal text] | [Command failed] | [stderr prefix `ERROR`, nonzero exit code, and remediation line] | [Not Started] |
| [API docs response examples] | [Colored badges] | [HTTP status class] | [Status code and label text in badge] | [Not Started] |
| [Data import review] | [Highlighted rejected rows] | [Rows blocked from import] | [Rejected-row table, count summary, downloadable CSV] | [Not Started] |
...
| Cognitive load documentation | Comprehensive | Per game system | Not Started | For each system in module-index.md, document the maximum number of things it asks the player to simultaneously track. Flag any system where the number exceeds 4. This is not a hard rule but a review trigger — high cognitive load systems need compensating UI clarity. See Per-Feature Accessibility Matrix below. |
...
> are addressed. When a new system is added to module-index.md, a row must be
...
| [Add system from module-index.md] | | | | | | |
