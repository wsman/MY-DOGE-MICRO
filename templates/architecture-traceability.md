...
<!-- One row per technical requirement extracted from a CDD.
     A "technical requirement" is any CDD statement that implies a specific
...
| Req ID | CDD | System | Requirement Summary | ADR(s) | Status | Notes |
|--------|-----|--------|---------------------|--------|--------|-------|
| TR-[cdd]-001 | [filename] | [system name] | [one-line summary] | [ADR-NNNN] | ✅ | |
| TR-[cdd]-002 | [filename] | [system name] | [one-line summary] | — | ❌ GAP | Needs `/architecture-decision [title]` |
...
- [ ] TR-[id]: [requirement] — CDD: [file] — Suggested ADR: "[title]"
...
## ADR → CDD Coverage (Reverse Index)

<!-- For each ADR, which CDD requirements does it address? -->

| ADR | Title | CDD Requirements Addressed | Engine Risk |
|-----|-------|---------------------------|-------------|
...
<!-- Requirements that existed in a CDD when an ADR was written, but the CDD
     has since changed. The ADR may need updating. -->

| Req ID | CDD | Change | Affected ADR | Status |
|--------|-----|--------|-------------|--------|
...
**When writing a new ADR**: Add it to the "ADR → CDD Coverage" table and mark
the requirements it satisfies as ✅ in the matrix.

**When approving a CDD change**: Scan the matrix for requirements from that CDD
