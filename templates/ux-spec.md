...
> **Related CDDs**: [Links to the CDD sections that generated this UI requirement — e.g., `design/cdd/inventory.md § UI Requirements`]
...
>
> **Product adaptation**: For general product work, use this same template for
> web screens, desktop/mobile flows, CLI command flows, API consumer journeys, and
> data workflow handoffs. Keep the same level of specificity, but replace
> player/game terms with user/workflow/system-contract terms where appropriate.
> Examples: `SignupFlow`, `ImportCsvWorkflow`, `cli deploy`, `POST /invoices`
> consumer flow, `DatasetReviewScreen`.
...
**Product equivalent — user need**:

[For product screens/flows, name the user's job and the friction this surface removes.
Example — web: "Lets an admin invite a teammate without needing to understand role
internals." Example — CLI: "Lets a developer deploy the current package and understand
whether the deployment is queued, running, or failed." Example — API: "Lets an
integrator create an invoice with predictable validation errors and an idempotent retry
path." Example — data workflow: "Lets an analyst import a CSV, inspect rejected rows,
and commit only clean records."]

**The product/user goal**:

[One sentence. Example: "Complete the import, resolve validation errors, and confirm
the number of accepted rows without leaving the workflow."]

**The system contract goal**:

[One sentence. Example: "Emit `ImportCompleted`, persist the accepted row count, and
return machine-readable validation errors for rejected rows."]
...
**Product arrival context examples**:

| Product Surface | User Just Did | Cognitive Load | Likely Goal | Likely Fear |
|----------------|---------------|----------------|-------------|-------------|
| Web app screen | Opened billing settings from an invoice failure banner | Medium — they know payment failed but not why | Update payment method and clear the failed invoice state | Being double-charged or losing service |
| CLI command flow | Ran `tool deploy --prod` from a dirty branch | High — production risk and terminal-only feedback | Understand whether deployment is blocked and how to fix it | Accidentally shipping the wrong commit |
| API consumer flow | Received a 422 from `POST /invoices` | Medium — mapping local data to remote validation rules | Correct payload fields without trial-and-error | Silent partial creation or duplicate invoices |
| Data workflow | Uploaded a CSV with mixed valid/invalid rows | High — many records and irreversible import risk | Review rejected rows and commit clean records | Corrupting production data |
...
**Product entry/exit examples**:

| Surface | Entry Trigger | Data Passed In | Exit / Result | Contract Notes |
|---------|---------------|----------------|---------------|----------------|
| Web workflow | User clicks "Import CSV" from dataset page | Dataset ID, allowed columns, current schema version | Import job created or draft discarded | Must preserve row-level validation results until user confirms |
| CLI flow | User runs `tool init --template api` | Working directory, selected template, package manager | Files scaffolded or no-op with reason | Must print next command and nonzero exit code on failure |
| API flow | Client calls `POST /tokens` | Credentials, scopes, idempotency key | Token response or structured error | Must document retry behavior and error codes |
| Data pipeline | Scheduled sync starts | Source cursor, destination schema, migration version | Cursor advanced or rollback recorded | Must be resumable without duplicate writes |
...
| [e.g., What is the game's item drop policy — permanent loss or drop-to-world?] | [systems-designer] | [Requires CDD update] | [Blocked on inventory CDD Edge Cases section] |
