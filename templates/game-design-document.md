> **Historical game-specific template.**
>
> The current unified workflow writes CDDs under `design/cdd/`, but this GDD
> template is intentionally preserved as a game-development reference. Do not
> delete its game-specific examples, notes, or sections. If a future workflow no
> longer uses this file directly, move the content to the reference archive
> rather than replacing it with product-only guidance.
...
|--------------------------|-----------|----------------------------|--------|
| [e.g., "combo multiplier feeds score"] | `design/cdd/score.md` | `combo_multiplier` output value | Data dependency |
| [e.g., "death triggers respawn"] | `design/cdd/respawn.md` | Death state transition | State trigger |
| [e.g., "stamina gates dodge"] | `design/cdd/stamina.md` | Stamina depletion rule | Rule dependency |
