...
- [Research Copilot CLI：`doge session` / `doge run`](#research-copilot-clidoge-session--doge-run)
...
| **Research Copilot CLI** | `doge session` / `doge run` | `agent_state.db` + Tool Registry | embedded 模式本地持久化；gateway 模式通过 SDK/v1 访问 daemon |
...
| [`run`](#research-copilot-clidoge-session--doge-run) | 执行一次持久化 agent run | `question`、`--session`、`--json`、`--trace`、`--follow`、`--jsonl` | `src/doge/interfaces/cli/commands/run.py` |
...
## Research Copilot CLI：`doge session` / `doge run`
