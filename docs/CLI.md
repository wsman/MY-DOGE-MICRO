...
| **Research Copilot CLI** | `doge session` / `doge run` | `agent_state.db` + Tool Registry | 本地持久化 agent session/run，无 HTTP 依赖 |
...
| [`session`](#research-copilot-clidoge-session--doge-run) | 创建/恢复 Research Copilot 会话 | `--title`、`--resume`、`--message` | `src/doge/interfaces/cli/commands/session.py` |
| [`run`](#research-copilot-clidoge-session--doge-run) | 执行一次持久化 agent run | `question`、`--session`、`--json`、`--trace` | `src/doge/interfaces/cli/commands/run.py` |
