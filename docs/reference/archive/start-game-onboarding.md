# Historical `/start` Game Onboarding Reference

Source: `.claude/skills/start/SKILL.md` before `/constitute` became the unified
project entry point.

This file preserves the former game-specific onboarding command in full. It is
not an active slash command and should not be restored under `.claude/skills/`
unless `/start` is intentionally reintroduced. Use `/constitute` as the active
project entry point.
...
Check:
- **Engine configured?** Read `standards/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
- **Game concept exists?** Check for `design/cdd/game-concept.md`.
- **Source code exists?** Glob for source files in `src/` (`*.gd`, `*.cs`, `*.cpp`, `*.h`, `*.rs`, `*.py`, `*.js`, `*.ts`).
- **Prototypes exist?** Check for subdirectories in `prototypes/`.
- **Design docs exist?** Count markdown files in `design/cdd/`.
...
- **Prompt**: "Welcome to historical pre-CDD game onboarding! Before I suggest anything, I'd like to understand where you're starting from. Where are you at with your game idea right now?"
- **Options**:
...
- **User is returning (engine configured, concept exists)**: Skip onboarding entirely — "It looks like you're already set up! Your engine is [X] and you have a game concept at `design/cdd/game-concept.md`. Review mode: `[read from production/review-mode.txt, or 'lean (default)' if missing]`. Want to pick up where you left off? Try `/sprint-plan` or just tell me what you'd like to work on."
