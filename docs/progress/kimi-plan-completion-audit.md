...
|---|---|---|---|
| S017-002 | Live Kimi smoke execution | `passed` | Runner/validator and redacted blocked evidence exist at `scripts/run_kimi_live_smoke.py`, `scripts/validate_kimi_live_smoke_evidence.py`, and `production/qa/evidence/live/kimi-live-smoke-2026-06-22.json`; strict closure requires a live run to replace the same evidence file with required text + Vision passed scenarios, optional Files and Agent SDK status, true live env gates, and per-required-scenario usage summaries. If Files succeeds, the validator still requires a redacted file id hash and Files cleanup confirmation. Real execution still requires `MOONSHOT_API_KEY`, `DOGE_LIVE_KIMI=1`, live network/spend, a normal-size Vision image, and optional SDK install. |
