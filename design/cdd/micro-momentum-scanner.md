...
This is the single source of truth. The DuckDB views `vw_rsrs_ranking_cn/us` (Module #2; canonical version-controlled DDL at `src/doge/infrastructure/database/views.sql`, mirror at `data/views.sql`) reproduce this formula in SQL — sign and magnitude now match the Python path for all series including monotonic uptrends/downtrends (S003-005; see notes below).
...
>
> **RESOLVED (S003-005, 2026-06-12):** the DuckDB view `vw_rsrs_ranking_cn/us` previously computed `ROW_NUMBER() OVER (... ORDER BY date DESC) AS rn` (rn=1 is newest) and then `REGR_SLOPE(rn, close)`, whose sign was INVERTED relative to the Python scalar path (`x = arange(0..17)` oldest→newest) for every monotonic series (a perfectly increasing series yielded RSRS **−1.0** from the view but **+1.0** from Python). This was independent of the zero-boundary convention above. The fix: the view now regresses on an ascending time index `ROW_NUMBER() OVER (... ORDER BY date ASC) AS rn_asc` (rn_asc=1=oldest), so the view and Python agree on sign and magnitude (verified to 1e-5 on real uptrend and downtrend tickers). The downstream liquidity / 60-day-change CTEs keep the legacy DESC `rn` (rn=1=newest); only the regression's time index changed. The view DDL was also brought under version control at `src/doge/infrastructure/database/views.sql` (the gitignored `data/views.sql` remains as a backward-compat mirror). Regression guard: `tests/migration/test_rsrs_view_sign_convention.py::test_perfectly_increasing_view_is_positive_one` (the strict `xfail` was removed and is now a hard assertion).
