...
- **#8 `mcp-server`** — `mcp_server.py:346` reads `stock_notes` directly (`SELECT created_at, content FROM stock_notes WHERE ticker=? ORDER BY created_at DESC LIMIT 5`) to enrich MCP tool responses. **NOTE**: this MCP query does NOT filter `deleted_at` — an open consistency gap (§9).
...
- [ ] `mcp_server.py:346` notes query filters `deleted_at IS NULL` (OPEN — §9 consistency gap).
...
4. **`mcp_server.py:346` does not filter `deleted_at`** — soft-deleted notes leak into MCP tool responses. The MCP module (#8) must be updated when this CDD's soft-delete contract is ratified.
