...
0. **Domain detection**: Check for concept documents in `design/cdd/`:
...
> "This sprint has no QA plan. A sprint plan without a QA plan means test requirements are undefined — developers won't know what 'done' looks like from a QA perspective, and the sprint cannot pass the **[Game]** Production → Polish / **[Product]** Implementation → Verification gate without one.
>
...
- Options:
  ...
  - `[B] Skip for now — I understand QA sign-off will be blocked at the [Game] Production → Polish / [Product] Implementation → Verification gate`
...
> before the last story is implemented. The **[Game]** Production → Polish / **[Product]** Implementation → Verification gate requires a QA
