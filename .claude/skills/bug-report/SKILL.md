...
- When to use: Creates a structured bug report from a description, or analyzes code to identify potential bugs. Ensures every bug report has full reproduction steps, severity assessment, and context.
- Inputs: Command arguments: `/bug-report [description] | analyze [path-to-file]`; project artifacts referenced below; user decisions and approvals before writes.
...
Detect the project domain before writing the report:
- `design/cdd/game-concept.md` -> **[Game]** use game terms where relevant: build, scene, level, platform, input method, player impact, save file, multiplayer state, and playtest source.
- `design/cdd/product-concept.md` -> **[Product]** use product terms: service/app version, endpoint/command/page, user role, environment, logs, request ID, migration state, data impact, and workflow impact.
- If no concept doc exists, keep the report domain-neutral and ask for the affected surface.

Preserve game bug examples. Product fields are extra context for non-game projects.
