...
- `design/cdd/game-concept.md` -> **[Game]** localize player-facing strings, dialogue, HUD/UI labels, item names, tutorials, platform metadata, and in-engine text behavior.
- `design/cdd/product-concept.md` -> **[Product]** localize web UI, CLI help/errors, API error messages, docs, onboarding copy, email/notification text, locale headers, and message catalogs.
- If unclear, ask whether strings are game-facing or product/user/developer-facing.

Keep all game localization checks, including engine-specific notes. Product i18n checks are additional.
...
Read:
- `design/cdd/` — extract game genre, tone, setting, character names
...
Read `standards/technical-preferences.md` to determine the engine. Then check:
...
- The list of all screens/flows in the game (from `design/cdd/` or `/content-audit` output)
