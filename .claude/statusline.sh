#!/usr/bin/env bash
# Constitution Driven Development -- Status Line
...
if [ -z "$stage" ]; then
  ...
  tech_configured=false
  src_count=0

  { [ -f "$concept_file" ] || [ -f "$product_concept_file" ]; } && has_concept=true
  [ -f "$product_concept_file" ] && [ ! -f "$concept_file" ] && product_domain=true
  [ -f "$systems_file" ] && has_systems=true

  # Check if engine or product stack is configured (not placeholder)
  if [ -f "$tech_prefs" ]; then
    tech_line=$(grep -m1 -E '^[[:space:]]*(-[[:space:]]*)?\*\*(Engine|Language|Framework|Stack)\*\*:' "$tech_prefs" 2>/dev/null || true)
    if [ -n "$tech_line" ] && ! echo "$tech_line" | grep -Eq "TO BE CONFIGURED|Not selected|TBD|\\[.*\\]"; then
      tech_configured=true
    fi
  fi

  # Count source files (language-agnostic)
  if [ -d "$cwd/src" ]; then
    src_count=$(find "$cwd/src" -type f \( -name "*.gd" -o -name "*.cs" -o -name "*.cpp" -o -name "*.c" -o -name "*.h" -o -name "*.hpp" -o -name "*.py" -o -name "*.js" -o -name "*.jsx" -o -name "*.ts" -o -name "*.tsx" -o -name "*.rs" -o -name "*.go" -o -name "*.lua" -o -name "*.java" -o -name "*.rb" -o -name "*.php" -o -name "*.tscn" -o -name "*.tres" \) 2>/dev/null | wc -l | tr -d ' ')
  fi
  ...
  if [ "$product_domain" = true ]; then
    if [ "$src_count" -ge 10 ] 2>/dev/null; then
      stage="Implementation"
    elif [ "$has_adrs" = true ] || [ "$tech_configured" = true ]; then
    ...
    elif [ "$tech_configured" = true ]; then
...
breadcrumb=""
if [ "$stage" = "Production" ] || [ "$stage" = "Implementation" ] || [ "$stage" = "Polish" ] || [ "$stage" = "Verification" ] || [ "$stage" = "Release" ]; then
