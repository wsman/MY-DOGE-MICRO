...
elif [ -f "design/cdd/game-concept.md" ]; then
    PROJECT_DOMAIN="game"
fi

# Check design documents for required sections
DESIGN_FILES=$(echo "$STAGED" | grep -E '^design/cdd/')
if [ -n "$DESIGN_FILES" ]; then
    while IFS= read -r file; do
        if [[ "$file" == *.md ]] && [ -f "$file" ]; then
            if [[ "$file" == *"product-concept.md" ]]; then
                REQUIRED_SECTIONS=("Core Identity" "User Journey" "Principles" "Target Users" "Scope" "Risks" "Next Steps")
            elif [ "$PROJECT_DOMAIN" = "product" ] && [[ "$file" != *"game-concept.md" ]]; then
                REQUIRED_SECTIONS=("Overview" "User Promise" "Detailed Design" "Data Model" "Edge Cases" "Dependencies" "Configuration" "Integration" "Acceptance Criteria")
            else
