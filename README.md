...
> **Agent Runtime Maturity**：Research Copilot runtime 按 Level 1/2/3 管理成熟度。只有当 [docs/progress/runtime-maturity.yaml](docs/progress/runtime-maturity.yaml) 中对应 level 的 required gates 都有代码和测试证据时，README 或发布说明才可标记该 level 为 stable。当前实现提供 Level 1/2/3 的开发切片与验证入口，不把未来生产能力提前包装成已完成 SLA。
