...
| [docs/API.md](docs/API.md) | FastAPI HTTP 接口参考（127.0.0.1:8901，51 个 product routes、SSE 契约、错误码） |
| [docs/CLI.md](docs/CLI.md) | 命令行参考（推荐 `doge ...`；旧 `python src/cli.py ...` 仅作为兼容 shim） |
...
| [design/cdd/module-index.md](design/cdd/module-index.md) | 模块设计契约（15 个模块：市场数据、Research Copilot、Document Evidence Pipeline、SDK/Daemon 等） |
| [design/ux/](design/ux/) | UX 流程规格与可访问性基线（按视图逐页） |
| [docs/architecture/runtime-levels.md](docs/architecture/runtime-levels.md) | Research Copilot Level 1/2/3 runtime 边界 |
| [docs/progress/runtime-maturity.yaml](docs/progress/runtime-maturity.yaml) | runtime 成熟度和禁止声明生产就绪的机器可读状态 |
...
> **Agent Runtime Maturity**：Research Copilot runtime 按 Level 1/2/3 管理成熟度。只有当 [docs/progress/runtime-maturity.yaml](docs/progress/runtime-maturity.yaml) 中对应 level 的 required gates 都有代码和测试证据时，README 或发布说明才可提升该 level 的成熟度标签。当前 `production_ready: false`，只表示已有 Level 1/2/3 的开发切片与验证入口，不把未来生产能力提前包装成已完成 SLA。
...
-   **配置解耦**:
    -   黑名单、流动性阈值、窗口长度等参数统一由 `Settings().market`（`src/doge/config/settings.py` 的 `MarketConfig`）管理，是单一配置来源（S002-008 / TR-019，遵循 ADR-0002）。
    -   `models_config.json` 不再承载 `scanner_filters` 节；动量扫描器直接读取 `get_settings().market`，避免配置漂移。

### AI 行业分析 (Industry Analysis)

-   **位置**: 当前 canonical runtime 位于 `src/doge/application/agent_runtime/`、`src/doge/interfaces/cli/`、`src/doge/interfaces/api/routers/{agent,documents,v1}/`；旧 `src/micro/industry_analyzer.py` 仅作为历史/兼容路径出现在旧报告和迁移文档中。
-   **工作流程**:
...
> **注（S002-008 / TR-019）**：动量扫描器的筛选参数（`us_blacklist`、`min_volume_cn/us`、`max_change_pct`、`rsrs_window`）已从 `models_config.json` 移出，统一由 `Settings().market`（`src/doge/config/settings.py` 的 `MarketConfig`）作为单一来源管理，遵循 ADR-0002。`models_config.json` 不再包含 `scanner_filters` 节。
...
| `Settings().market.*` | `us_blacklist` | 美股杠杆/反向ETF黑名单（≈52只） | `Settings().market.us_blacklist`（见 `src/doge/config/settings.py`） |
| | `cn_min_volume` / `us_min_volume` | A股/美股最低日均成交额阈值 | 200000000 RMB / 20000000 USD |
| | `max_change_pct` | 最大涨跌幅过滤（防止虚假暴涨，仅美股） | 400 |
| | `rsrs_window` | RSRS计算窗口（交易日数） | 18 |
| | `cn_universe_prefixes` | A股可投资代码前缀 | `("00","30","60","68")` |
...
-   **目录结构**:
    ...
    -   `doge ...`: 查询 CLI（DuckDB 只读分析；`src/cli.py` 是兼容 shim）
