...
class ContextBuilder:
    ...
    def build(self, run: AgentRun, events: list[AgentEvent]) -> list[AgentMessage]:
        ...
        document_ids = self._authorized_document_ids(run.document_ids)
        messages.extend(self._build_document_messages(document_ids))
        messages.extend(self._build_session_history(run))
    ...
    def _system_prompt(self) -> str:
        ...
        if self._enterprise_context is not None:
            prompt += (
                f" Data classification: {self._enterprise_context.data_classification}. "
                "Do not reveal tenant, user, or portfolio identifiers beyond the "
                "authorized context provided in this run."
            )
        return prompt

    def _authorized_document_ids(self, document_ids: list[str]) -> list[str]:
        if self._enterprise_context is None:
            return document_ids
        return [
            document_id for document_id in document_ids
            if self._enterprise_context.can_access_document(document_id)
        ]

    def _build_document_messages(self, document_ids: list[str]) -> list[AgentMessage]:
        messages: list[AgentMessage] = []
        document_context = self._build_document_context(document_ids)
        if document_context:
            messages.append(AgentMessage(role="system", content=document_context))
        messages.extend(self._build_multimodal_document_messages(document_ids))
        return messages

    def _build_multimodal_document_messages(self, document_ids: list[str]) -> list[AgentMessage]:
    ...
    def _build_session_history(self, run: AgentRun) -> list[AgentMessage]:
        ...
        for turn in sorted(session.turns, key=lambda item: item.created_at):
            ...
            if turn.run_id and artifact_count < self._max_prior_artifacts:
                prior_run = self._runs.get(turn.run_id)
