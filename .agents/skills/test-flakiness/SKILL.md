...
- When to use: Detect non-deterministic (flaky) tests by reading CI run logs or test result history. Aggregates pass rates per test, identifies intermittent failures, recommends quarantine or fix, and maintains a flaky test registry. Best run during Polish phase or after multiple CI runs.
- Inputs: Command arguments: `/test-flakiness [ci-log-path | scan | registry]`; project artifacts referenced below; user decisions and approvals before writes.
...
Detect the project domain before analyzing flakiness:
- `design/cdd/game-concept.md` -> **[Game]** keep engine/test-runner parsing for Godot, Unity, Unreal, gameplay timing, orphaned nodes/objects, scene setup, and resource cleanup.
- `design/cdd/product-concept.md` -> **[Product]** analyze product test flakiness across pytest, Vitest/Jest, cargo test, go test, Playwright, JUnit XML, GitHub Actions logs, network/service timing, migrations, and shared test data.
- If unclear, ask which test runner and domain should drive parsing.

Do not delete engine-specific flake guidance. Product runner guidance is added beside it.
