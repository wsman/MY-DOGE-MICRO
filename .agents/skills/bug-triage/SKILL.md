...
- `design/cdd/game-concept.md` -> **[Game]** prioritize by player impact, progression blockers, platform certification risk, save corruption, multiplayer fairness, and playtest frequency.
- `design/cdd/product-concept.md` -> **[Product]** prioritize by user impact, data loss, security/privacy, uptime, API/CLI breakage, migration risk, customer segment, and operational severity.
- If unclear, ask whether severity should follow game/player impact or product/user impact.

Keep existing game severity logic; add product severity logic only as a parallel branch.
# Bug Triage
...
Read `standards/coding-standards.md` for severity/priority definitions if they
