...
- **ADR-0007 (CORS hardening) deferral is sound while loopback bind holds** — `src/doge/interfaces/api/main.py` `allow_origins=['*']` is permissive; `src/doge/interfaces/api/main.py` `uvicorn.run(app, host='127.0.0.1', port=8901)` is loopback-only. The ADR's "Deferral Decision (S003-013)" block makes the security argument explicit and forbids non-loopback exposure. The error-envelope half is DONE (two global handlers at `src/doge/interfaces/api/main.py,69`; contract regression at `tests/contract/test_api_error_envelope.py`).
...
- **scan.py imports infrastructure-layer adapter and runs raw SQL across the layer boundary (concern)** — `src/doge/interfaces/api/routers/scan.py` `from doge.infrastructure.database.sqlite import SQLiteConnection`; `:205-207` `SQLiteConnection(db_path).execute("SELECT DISTINCT ticker FROM stock_prices")`; `:27,29` imports `composition.refresh_views` and `sqlite_storage.SQLiteStorageRepository`. Adapter at `src/doge/infrastructure/database/sqlite.py:6,30` does `sqlite3.connect` internally. Bypasses the `deps.py` DI seam.
- **notes.py delegates every handler to a legacy module doing sqlite3.connect + connect_duckdb directly (concern)** — `src/doge/interfaces/api/routers/notes.py,36,50,57,63,75` all call into `src.ai_analysis.stock_notes`; callee `src/ai_analysis/stock_notes.py:14` imports `connect_duckdb`, `:25` `sqlite3.connect(NOTES_DB)`, raw SQL at `:38,41,57,86,112,128,131,137,154,159,188,205,223,241`. Transitive coupling not surfaced by the §6 grep.
...
- **Macro/scan legacy-module coupling (nit, Wave-5)** — `src/doge/interfaces/api/routers/macro.py` imports `GlobalMacroLoader`/`DeepSeekStrategist`/`save_macro_report` from `src.macro`/`src.micro`; `src/doge/interfaces/api/routers/scan.py,277` import legacy `src.micro`. Permitted under control-manifest §2 legacy-tolerance clause.
...
Advisory nits only (none block release): `docs/CLI.md:247-251` exit-code table is stale (describes 0-on-no-data vs shipped EXIT_NO_DATA=1); SSE in-band `progress=-1` carries raw `str(e)` at `src/doge/interfaces/api/routers/scan.py` and `src/doge/interfaces/api/routers/macro.py` (out of HTTP-envelope scope, loopback-only); 422 `RequestValidationError` intentionally NOT enveloped at `src/doge/interfaces/api/main.py` (tracked S002-011); macro CLI mixes English exception text with Chinese remediation hint (`src/macro/cli.py:109-111`); server-side `_scan_status` map exposes `idle|running` only (`src/doge/interfaces/api/routers/scan.py,160,257`), terminal `error|complete` live client-side only.
...
8. **SSE in-band `progress=-1` carries raw `str(e)`** — `src/doge/interfaces/api/routers/scan.py`, `src/doge/interfaces/api/routers/macro.py`. Route through operator-safe fixed string or add redaction pass; document chosen behavior in `docs/API.md §SSE Contract`. Loopback-only, low release risk.
