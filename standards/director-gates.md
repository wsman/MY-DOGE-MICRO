...
Set once during `/constitute`. Edit the file directly to change it at any time.
...
- Gate: [GATE-ID] (see standards/director-gates.md)
...
Agent: `creative-director` | Model tier: Opus | Domain: Vision, game pillars/product principles, player/user experience
...
or any time pillars are revised). For product projects, after product principles,
anti-principles, and the core user promise are defined.

**Context to pass**:
- Full pillar set with names, definitions, and design tests
- Product principle set with names, definitions, and design tests (if product)
- Anti-pillars list
- Core fantasy statement
- User Promise and JTBD statement (if product)
...
**Product prompt extension**:
> "For product projects, review the product principles and User Promise instead of
> game pillars/core fantasy. Are they falsifiable against real workflow decisions?
> Do they create useful tension between user value, technical constraints, and scope?
> Does the JTBD statement identify a concrete job, situation, motivation, and outcome?
> Return the same verdict scale."

**Verdicts**: APPROVE / CONCERNS / REJECT

---

### CD-GDD-ALIGN — CDD Pillar Alignment Check

**Trigger**: After a system CDD is authored (design-system, quick-design, or any
workflow that produces a CDD)

**Context to pass**:
- CDD file path
- Game pillars (from `design/cdd/game-concept.md` or `design/cdd/principles.md`)
- MDA aesthetics target for this game
- System's stated Player Fantasy section
- Product principles and User Promise (from `design/cdd/product-concept.md`, if product)
- JTBD and primary workflow this module supports (if product)
- Module's stated User Promise section (if product)

**Prompt**:
> "Review this system CDD for pillar alignment. Does every section serve the stated
...
**Product prompt extension**:
> "For product CDDs, review alignment against product principles, User Promise,
> JTBD, and the primary workflow. Does every section reduce real user friction or
> protect an explicit product principle? Do API/CLI/web/data behaviors contradict
> the promised workflow? Return APPROVE, CONCERNS [specific sections with issues],
> or REJECT [principle/user-promise violations that must be redesigned]."
...
**Trigger**: After the module index is written by `/map-systems` — validates the
complete system set before CDD authoring begins

**Context to pass**:
- Module index path (`design/cdd/module-index.md`)
- Game pillars and core fantasy (from `design/cdd/game-concept.md`)
- Product principles, User Promise, and JTBD (from `design/cdd/product-concept.md`, if product)
- Priority tier assignments (MVP / Vertical Slice / Alpha / Full Vision)
- Product priority tiers (MVP / v1 / v2+) and primary workflow validation target (if product)
...
> the decomposition misses critical design intent and must be revised before CDD
> authoring begins]."

**Product prompt extension**:
> "For product projects, review the module decomposition against product principles,
> User Promise, JTBD, and workflow validation target. Does the MVP module set
> collectively let the user complete the promised job? Are there API/CLI/web/data
> modules missing from the workflow? Are any modules scope creep because they serve
> no product principle or user outcome? Return the same verdict scale."
...
**Trigger**: After narrative CDDs, lore documents, dialogue specs, or world-building
...
- Product validation report path (if product)
- Product principles, User Promise, JTBD, and workflow being tested (if product)
...
**Product prompt extension**:
> "For product validation reports, review whether the user-test or workflow
> validation proves the User Promise and JTBD. Did users complete the target job?
> Where did API/CLI/web/data workflow friction block adoption or trust? Return
> APPROVE (workflow promise is landing), CONCERNS [gaps], or REJECT [user promise
> is not validated — redesign needed before implementation continues]."
...
- Product principles, User Promise, JTBD, and workflow validation evidence (if product)
...
> any design decisions across CDDs or architecture that compromise the intended
> player experience? Return READY, CONCERNS [list], or NOT READY [blockers]."

**Product prompt extension**:
> "For product projects, assess whether product principles, User Promise, JTBD,
> and workflow validation are faithfully represented in concept, CDDs, architecture,
> stories, and validation artifacts. Return READY, CONCERNS, or NOT READY."
...
Agent: `technical-director` | Model tier: Opus | Domain: Architecture, engine/stack risk, performance
...
CDD authoring begins — validates that the system structure is architecturally
sound before teams invest in writing CDDs against it

**Context to pass**:
- Module index path (or the dependency map summary if index not yet written)
...
**Prompt**:
> "Review this systems decomposition from an architectural perspective before CDD
...
> (boundaries are architecturally sound — proceed to CDD authoring), CONCERNS
> [specific boundary issues to address in the CDDs themselves], or REJECT
> [fundamental boundary problems — the system structure will cause architectural
> issues and must be restructured before any CDD is written]."
...
- Product core workflow and JTBD (if product)
- Platform target
- Engine choice (or "undecided")
- Language/framework/stack choice (or "undecided", if product)
...
**Product prompt extension**:
> "For product projects, review technical risks for [workflow/API/CLI/web/data
> pipeline] targeting [platform] using [language/framework/stack or 'undecided
> stack']. Flag risks that could invalidate the User Promise or JTBD, stack-specific
> risks that should influence the technology choice, and commonly underestimated
> reliability, migration, security, and deployment risks. Return the same verdict
> scale."
...
- Stack reference path (`docs/reference/<stack>/VERSION.md`) and stack knowledge gap inventory (if product)
...
> (2) Are all HIGH risk engine or stack domains explicitly addressed or flagged as open
...
**Context to pass**:
- ADR file path
- Engine or stack version and knowledge gap risk level for the domain
- Version reference path (`docs/engine-reference/<engine>/VERSION.md` or `docs/reference/<stack>/VERSION.md`)
- Related ADRs (if any)
...
> Consequences section acknowledge the trade-offs honestly? Is the engine or stack
> version stamped? Are post-cutoff API risks flagged? Does it link to the CDD requirements
...
### TD-ENGINE-RISK — Engine/Stack Version Risk Review

**Trigger**: When making architecture decisions that touch post-cutoff engine or
framework APIs, or before finalizing any engine- or stack-specific implementation approach
...
- Product stack version and LLM knowledge cutoff (from `docs/reference/[stack]/VERSION.md`, if product)
...
**Product prompt extension**:
> "For product projects, apply the same review to the selected language, framework,
> SDK, package manager, database, or deployment platform reference under
> `docs/reference/<stack>/`. Verify the API exists in the pinned version and that
> deprecated or breaking changes have been handled."
...
- Engine reference path or Product stack reference path (`docs/reference/<stack>/VERSION.md`)
- ADR list

**Prompt**:
> "Review the current project state for [target phase] gate readiness from a
> technical direction perspective. Is the architecture sound for this phase? Are
> all high-risk engine or stack domains addressed? Are performance budgets realistic and
...
Agent: `art-director` | Model tier: Sonnet | Domain: Visual identity, art bible, product style guide, visual production readiness

---

### AD-CONCEPT-VISUAL — Visual Identity Anchor

**Trigger**: After game pillars are locked (brainstorm Phase 4), in parallel with
CD-PILLARS. For product projects, after product principles and the User Promise
are locked.

**Context to pass**:
- Game concept (elevator pitch, core fantasy, unique hook)
- Full pillar set with names, definitions, and design tests
- Product concept, User Promise, JTBD, and product principles (if product)
- Target platform (if known)
- Any reference games or visual touchstones mentioned by the user
- Product brand references, UI references, docs imagery references, or accessibility constraints mentioned by the user (if product)
...
**Product prompt extension**:
> "For product projects, propose 2-3 brand/UI visual identity directions. Each
> direction must include: (1) a one-line visual rule for product UI and docs,
> (2) design token direction for color/type/spacing, (3) component tone and icon
> language, (4) documentation imagery rules, and (5) accessibility constraints.
> Recommend the direction that best serves the User Promise and product principles."
...
- Game pillars and core fantasy
- Product principles, User Promise, and JTBD (if product)
- Platform and performance constraints (from `standards/technical-preferences.md` if configured)
- Visual identity anchor chosen during brainstorm (from `design/cdd/game-concept.md` or `design/cdd/product-concept.md`)
...
**Product prompt extension**:
> "For product style guides, verify brand promise, UI design tokens, component
> tone, documentation imagery, and accessibility constraints are specific enough
> for implementation and docs production. Return the same verdict scale."
...
- Visual identity anchor from the concept document (if present)
- Art bible path if it exists (`design/art/art-bible.md`)
- Product style guide, design token, UI spec, or docs imagery artifacts if they exist
...
**Product prompt extension**:
> "For product projects, evaluate whether visual standards needed for the current
> phase are documented: brand/UI style guide, design tokens, UI specs, docs imagery,
> accessibility constraints, and handoff notes for implementation. Return READY,
> CONCERNS, or NOT READY."
...
- ADR list with statuses
- Engine or stack reference path and selected language/framework

**Prompt**:
> "Review this architecture for implementation feasibility. Flag: (a) any decisions
> that would be difficult or impossible to implement with the stated engine/stack and
...
> that contradict standard [engine/stack] idioms. Return FEASIBLE, CONCERNS [list], or
...
> testable and documented? Are there any correctness issues against the CDD rules?
...
- The CDD requirement (TR-ID and text) the story covers
- Product workflow/API/CLI/data path and User Promise/JTBD link when the story is product-facing
...
> DEFERRED, not BLOCKED). For product stories, require the acceptance criteria to
> name the observable API/CLI/web/data behavior, migration/config result, or workflow
> validation evidence. Return ADEQUATE (criteria are implementable as written),
...
- CDD acceptance criteria for the system
- Product contract, migration, workflow validation, and smoke evidence paths (if product)
...
> tests, documented playtests, or product workflow validation evidence? Are the CDD acceptance criteria each mapped to at
> least one test? Are there untested edge cases from the CDD Edge Cases section?
...
|-------|---------------|----------------|
| **Concept** | CD-PILLARS, AD-CONCEPT-VISUAL | TD-FEASIBILITY, PR-SCOPE, AD-ART-BIBLE |
| **Systems Design** | TD-SYSTEM-BOUNDARY, CD-SYSTEMS, PR-SCOPE, CD-GDD-ALIGN (per CDD) | ND-CONSISTENCY, AD-VISUAL |
