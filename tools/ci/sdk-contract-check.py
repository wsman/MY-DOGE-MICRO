...
CHECKS: tuple[SurfaceCheck, ...] = (
    ...
    SurfaceCheck(
        ...
        "GET",
        "/v1/slots",
        ("def list_slots(self)", '"/v1/slots"'),
        ("listSlots()", "'/v1/slots'"),
        ("listSlots()", "dogeClient.platform.listSlots"),
    ),
    SurfaceCheck(
        "slot get",
        "GET",
        "/v1/slots/{slot_id}",
        ("def get_slot(self, slot_id: str)", 'f"/v1/slots/{slot_id}"'),
        ("getSlot(slotId: string)", "`/v1/slots/${slotId}`"),
    ),
    SurfaceCheck(
        "slot install",
        "POST",
        "/v1/slots/install",
        ("def install_slot(self, source: str)", '"/v1/slots/install"'),
        ("installSlot(source: string)", "'/v1/slots/install'"),
        ("installSlot(payload", "dogeClient.platform.installSlot"),
    ),
    SurfaceCheck(
        "workspace create",
        "POST",
